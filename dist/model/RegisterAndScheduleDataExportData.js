...
var RegisterAndScheduleDataExportData = /*#__PURE__*/function () {
   ...
   * @param format {String} Specifies the format of the file to export the messages to. Acceptable values are json and csv. (Default: json)
   * @param csvDelimiter {String} Sets a single character delimiter to separate the values in each row of the csv file which stores two-dimensional arrays of the exported message data. Either English alphabets or special characters can be used as a delimiter, including a horizontal tab (\\t), a line feed (\\n), a vertical bar (\\
   * @param timezone {String} Specifies the timezone to be applied to the timestamp of the exported messages. For example, US/Pacific, Asia/Seoul, Europe/London, etc. (Default: UTC)
   * @param senderIds {Array.<Number>} Specifies an array of the IDs of the users which are used to filter the messages by its sender for the export. This property is effective only when the data_type parameter is set to messages, and can be specified up to 10 IDs in the request. (Default: all messages sent by any user)
   * @param excludeSenderIds {Array.<Number>} Specifies an array of the IDs of the users which are used to exclude their sent messages from the export. This property is effective only when the data_type parameter is set to messages, and can be specified up to 10 IDs. (Default: all messages sent by any user)
   * @param channelUrls {Array.<String>} Specifies an array of one or more URLs of channels to export the messages from. This property is effective only when the data_type parameter is set to messages or channels. (Default: all channels)
   * @param excludeChannelUrls {Array.<String>} Specifies an array of one or more URLs of channels to exclude when exporting the messages. This property is effective only when the data_type parameter is set to messages or channels. (Default: include all channels)
   ...
   */
  function RegisterAndScheduleDataExportData(startTs, endTs, format, csvDelimiter, timezone, senderIds, excludeSenderIds, channelUrls, excludeChannelUrls, userIds, showReadReceipt, showChannelMetadata, neighboringMessageLimit) {
    _classCallCheck(this, RegisterAndScheduleDataExportData);

    RegisterAndScheduleDataExportData.initialize(this, startTs, endTs, format, csvDelimiter, timezone, senderIds, excludeSenderIds, channelUrls, excludeChannelUrls, userIds, showReadReceipt, showChannelMetadata, neighboringMessageLimit);
  }
  /**
  ...
  _createClass(RegisterAndScheduleDataExportData, null, [{
    key: "initialize",
    value: function initialize(obj, startTs, endTs, format, csvDelimiter, timezone, senderIds, excludeSenderIds, channelUrls, excludeChannelUrls, userIds, showReadReceipt, showChannelMetadata, neighboringMessageLimit) {
      obj['start_ts'] = startTs;
      obj['end_ts'] = endTs;
      obj['format'] = format;
      obj['csv_delimiter'] = csvDelimiter;
      obj['timezone'] = timezone;
      obj['sender_ids'] = senderIds;
      obj['exclude_sender_ids'] = excludeSenderIds;
      obj['channel_urls'] = channelUrls;
      obj['exclude_channel_urls'] = excludeChannelUrls;
      obj['user_ids'] = userIds;
      obj['show_read_receipt'] = showReadReceipt;
      obj['show_channel_metadata'] = showChannelMetadata;
      obj['neighboring_message_limit'] = neighboringMessageLimit;
    }
    /**
    ...
    value: function constructFromObject(data, obj) {
      if (data) {
        ...
        if (data.hasOwnProperty('sender_ids')) {
          obj['sender_ids'] = _ApiClient["default"].convertToType(data['sender_ids'], ['Number']);
        }

        if (data.hasOwnProperty('exclude_sender_ids')) {
          obj['exclude_sender_ids'] = _ApiClient["default"].convertToType(data['exclude_sender_ids'], ['Number']);
        }
